...
void copy_in_ghost_data(double* dest,
  ...
  bool is_leftmost, bool is_rightmost
) {
  if(not is_leftmost) {
...
int darma_main(int argc, char** argv)
{
  ...
  //
  sent_to_left.publish(n_readers=(!is_leftmost));
  sent_to_right.publish(n_readers=(!is_rightmost));
  ...
  for(int iter = 0; iter < n_iter; ++iter) {
    ...
    create_work(reads(left_ghost, unless=is_leftmost),
      ...
      copy_in_ghost_data(data_with_ghosts, left_ghost, data, right_ghost, is_leftmost, is_rightmost);
