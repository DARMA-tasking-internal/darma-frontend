...
int main(int argc, char** argv)
{
  ...
  auto gv_to_right = initial_access<double>("ghost_value_for_right_neigh", me, 0);
	...
	// publish for something that potentially goes off node	
 	gv_to_left.publish(n_readers=numMCSamples); 
 	gv_to_right.publish(n_readers=numMCSamples); // deleted after one read access handle is created

 	// missing version means this is the one time only it is published
 	// this key cannot be pusblished by anynody later
	...
	// set value of alpha for this iSampl
	auto alphaVecRead = read_access<std::vector<double>>("alpha_values", myPID);
	alphaVecRead.wait(); // syntax wrong, but just give idea that data has to be ready to be read here below
		...
		double alphadtOvdxSq = (alphaVecRead->data()[iSampl] * deltaT) / (deltaX * deltaX );   // alpha * DT/ DX^2
	 	...
	 	create_work([=]
	 	{
			for (int timeLoop = 0; timeLoop < n_iter; ++timeLoop)
			{
				...
				if (timeLoop == 0)
				{
			  		gv_from_left_neigh = read_access<double>("ghost_value_for_right_neigh", left_neighbor, 0);
			  		gv_from_right_neigh = read_access<double>("ghost_value_for_left_neigh", right_neighbor, 0);
				}
				else
				{
			  		gv_from_left_neigh = read_access<double>("ghost_value_for_right_neigh", left_neighbor, timeLoop, iSampl);
			  		gv_from_right_neigh = read_access<double>("ghost_value_for_left_neigh", right_neighbor, timeLoop, iSampl);	
				}
