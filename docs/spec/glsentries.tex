...
\newglossaryentry{garbage collection}
{
  ...
  description={Reclaiming (freeing) the memory associate with a variable after it is no longer required.
    This can occur automatically for reference-counted variables.}
}
...
\newglossaryentry{subtask}
{
  type=glossary,
  name=subtask,
  description={Any task instantiated with \codelink{create_work} will be a subtask of the task running
    at the time \codelink{create_work} was invoked.  For sequential semantics, a task cannot complete 
    until all of its subtasks have completed.}
}
...
\newglossaryentry{leaf task}
{
  type=glossary,
  name={leaf task},
  description={A task with no direct successors, i.e. at the end of a task graph branch},
}

\newglossaryentry{child task}
{
  type=glossary,
  name={child task},
...
\newglossaryentry{scheduling permissions}
{
  type=glossary,
  name={scheduling permissions},
  description={The permissions for a handle when scheduling new tasks,
   but which may not apply immediately. A task may schedule further tasks with read privileges
   in certain cases even if the data cannot be immediately read.}
}

\newglossaryentry{immediate permissions}
{
  type=glossary,
  name={immediate permissions},
  description={The permissions for a handle that applies immediately at the current point in execution.
    For immediate Read permissions, \inlinecode{handle.get_value()} can be called.
    For immediate Write permissions, \inlinecode{handle.set_value()} and \inlinecode{handle.get_reference()} can also be called.}
}
...
\newglossaryentry{archive}
{
  type=glossary,
  name=archive,
  description={In DARMA serialization, an object that performs either 1) packing operations, storing serialized values in the archive or 2) unpacking operations, deserializing values stored in the archive.},
}
...
\newglossaryentry{continuing context}
{
  ...
  description={the code in the outer scope after a \codelink{create_work}},
%  \begin{verbatim}
%  //outer scope predecessor context
%  create_work([=]{
%    //inner scope captured context
%  })
%  //outer scope continuing context
%  \end{verbatim}
%  },
}
...
\newglossaryentry{rank}
{
  ...
  name=rank,
  plural=ranks,
  description={A unique integer identifier for an \gls{execution stream} created in an SPMD launch.
  The term rank matches the MPI notion of a unique process ID in an MPI communicator}
}
...
\newglossaryentry{DHT}
{
  ...
  description={An implementation of a key-value map (table) that relies a consistent hash of
    keys and a partition of the key space to distribute storage of the table across a distributed system}
}
...
\newglossaryentry{task} %linked, defined
{
  type=glossary,
  name={task},
  description={The work unit explicitly instantiated by the application developer through \codelink{create_work}.
...
\newglossaryentry{operation} %linked, defined
{
  type=glossary,
  name={operation},
...
\newglossaryentry{fetch}
{
  type=glossary,
  name={fetch},
  description={A fetch operation reads values from the key-value store. Fetches are requests that must be satisfied with a matching publish into the key-value store. In DARMA, fetches implicitly occur when read-only handles are created through \codelink{read_access}.},
}
...
\newglossaryentry{task elasticity}
{
  type={glossary},
...
\newglossaryentry{concept}
{
  ...
  its concept checking using the \inlinecode{void_t} detection idiom~\cite{DetectionIdiom}.},
}
