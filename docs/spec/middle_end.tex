...
through the C++11 lambda mechanism and the copy {\it capture-default} (that is,
\inlinecode{[=]}).  The C++ standard specifies that if the copy
{\it capture-default} is given, any variables that are ODR-used\footnote{``one
definition rule''-used.  See
\url{http://en.cppreference.com/w/cpp/language/definition#ODR-use} for details. 
...
The tricks used to emulate keyword arguments in C++ are well-known and have been
...
C++11 and C++14 allow this to be done with rigorously zero runtime overhead ---
