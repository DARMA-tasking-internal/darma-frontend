...
\gls{migratable} units of work that enable the overlap of communication and 
computation as well as dynamic task reordering and load balancing strategies.
...
reusable components within the \gls{AMT} community.
...
models in order to facilitate exploration of the AMT design space. The
specification gives a programming model design that seeks to
clearly separate policy from mechanism. \todo{elaborate on what we mean by this} The set of policies specified by DARMA do not uniquely map to any
...
\CC compiler that supports the \CC11 standard with a small subset of \CC14
