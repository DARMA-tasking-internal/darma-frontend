...
algorithm~\cite{missingCitations}.}\todo{Jeremy: pointers to missing citations
you added}
...
\item [Execution stream:] An \gls{execution stream} will consist of a sequence
  ...
  \todo{Jeremy: should this be CDAG or task-dag?}  Each \gls{execution stream} is guaranteed to 
...
A \todo{Janine: add glossary entry here once Jeremy is done with pass on
section} DHT implementation of a \gls{key-value store} is not required by the specification,
...
\todo[inline]{David/Jeremy: currently we are omitting vasp appendix. Just
checking to make sure that is ok with you}
