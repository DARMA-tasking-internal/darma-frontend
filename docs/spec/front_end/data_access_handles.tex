...
\todo[inline]{Jeremy/David: I'm struggling some with the phrasing below.  I ist
  permissions that may be applied to a handle when the application developer is
instantiating tasks with create_work?}
\begin{itemize}
...
\glspls{tasks}) or Modify (\gls{handle} may be used in read-only or read-write
\gls{tasks}). 
\item[b] {\it Immediate}: Permissions that apply immediately, indicating the
  ``readiness'' of the \gls{handle}.  Immediate privileges can \todo{Jeremy:
  this is a typo, right?} ever be greater than Scheduling privileges.
  A \gls{task} can never have greater permissions doing immediate work than it can for instantiating deferred work.
  \inline{Jeremy/David: againg confusing phrasing, is it the handle or the task
  that has the permissions? Also, is it the task/application code applying the
permissions? Or, the translation layer? IMO, phrasing is off here on
the last sentence, becausa task should have permissions on one the work it is
immediately doing?  If my questions aren't clear, we can follow up via phone.  }
\end{itemize}
