...
\todo[inline]{ahandle command, when not wrapped in inlinecode, doesn't leave space between AccessHandle and the word that follows.}
...
\end{enumerate}
\todo[inline]{Change this - dependency management and coordination/communication between
...
\todo{sentence doesn't make sense}
...
\todo[inline]{Bring back the explanation of Type 1.  The code block below doesn't make sense without context.}
\begin{CppCode}
...
\cwork defers execution of the code block until the key-value store resolves the value of \inlinecode{float_handle} and converts it to a ready state.
...
\item[b] {\it Immediate}: Permissions that apply immediately, indicating the ``readiness'' of the handle.  Immediate privileges can never be greater than Scheduling privileges.
