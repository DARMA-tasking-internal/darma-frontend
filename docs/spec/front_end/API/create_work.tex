...
\codetarget{create_work} instantiates deferred work to be executed by the \gls{runtime
system}.
...
\begin{compactitem}
  \item \inlinecode{LambdaExpression} A \CC{}11 lambda expression with a copy
...
Currently \inlinecode{void} in the \specVersion-spec, but may be an object of unspecified
type in future implementations.

\paragraph{Details}\mbox{}\\
This function expresses work to be executed by the \gls{runtime system}.  Any
\codelink{AccessHandle<T>} variables used in the \inlinecode{LambdaExpression} will be
...
scheduling permissions available to the \codelink{AccessHandle<T>} at the time of
...
Most of the general restrictionsn and pitfalls related to \codelink{create_work}
