...
be called exactly once for each call of \textt{darma\_init} (which, in turn must
...
\paragraph{Restrictions and Pitfalls}
\begin{itemize}
  ...
  %\item \texttt{darma\_finalize} should be called in the same immediately
  %enclosing scope (IES) in which \texttt{darma\_init} was invoked.  
\end{itemize}
