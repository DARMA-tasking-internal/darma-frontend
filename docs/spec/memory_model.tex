...
Memory that is shared between \codelink{ranks} using a flat global memory space in
which data is identified by unique tuple identifiers, e.g. a \gls{key-value
  store} or \gls{tuple space} abstraction.  
  \todo{Jeremy/David: we need to figure out which of these terms we want to use, and do so consistently throughtout the text}
