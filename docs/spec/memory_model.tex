...
Within a \gls{DARMA} \codelink{rank}, memory is local or private, and the
standard \CC{} memory model applies. 
To share memory between \codelink{ranks} \gls{DARMA} uses a flat global memory space in
which data is identified by unique \gls{tuple} identifiers, e.g. a \gls{key-value
  store} or \gls{tuple space} abstraction.  
  \todo{Jeremy/David: we need to figure out which of these terms we want to use, and do so consistently throughout the text}
...
Within a \codelink{rank}, \todo{I changed this from within a task to within a
  rank, just vetting this} \glspl{handle} identifiers are resolved to a
specific local address and the standard \CC{} memory model applies.
\todo[inline]{handles and seq semantics needs more clarification, is it within
a rank that handles are resolved to a local address? within a rank does C++ 
memory model apply or is it within a task?}
...
Again, within tasks, the \CC{} \gls{memory model} applies. \todo{Should tasks
  in this sentence  be within a rank?}  
...
\codelink{rank} \todo{I changed process here to rank, please vet that is
  correct} (shared memory), but requires the application developer to specify
  \gls{coordination semantics} for objects exchanged between \codelink{rank} to ensure data consistency.
