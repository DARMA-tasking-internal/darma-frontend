...
(The third type of handle does not exist in the 0.2 version of the spec.)
...
modification priviledges on a block of data can exist at any given time.
...
This means that the same precautions should be taken as with asynchronous reads.  In particular, even if
the handle was in a modifiable state before the \inlinecode{publish}, it is no
