...
``wait_and_get_value()'' (or whatever we end up calling it) is an
...
if the \inlinecode{float} is on a remote node.
...
\inlinecode{create_work} block when possible.
...
\todo[inline]{publication versions}
...
crack at it?  His changes don't appear to be here\ldots} \gls{spmd} being the
