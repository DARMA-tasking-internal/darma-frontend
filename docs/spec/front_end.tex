...
2) a handle to data that might already exist and needs to
be read, or 
3) a handle to data that it wants to overwrite or modify. 

Type 1 is denoted as \inlinecode{initial_access} in DARMA, which informs the runtime that the data with the 
specified key does not yet exist, and the user intends to create this data and publish it. 
Hence, an \inlinecode{initial_access}
...
\end{CppCode}
The second type of handle, requesting read-only access to a piece of data, is via \inlinecode{read_access}. 
...
\end{CppCode}
The runtime guarantees that \inlinecode{get_value} will wait until the value exists and is ready locally.
This might involve moving data if the \inlinecode{float} is on a remote node.
In general, any calls to \inlinecode{get_value} should occur within a scoped code block to avoid dangling references to stale physical memory locations.
...
Better practice is to put deferred tasks inside a \inlinecode{create_work} block.

The third handle type, which informs the runtime that the data with the specified key will be read and overwritten or modified, 
is with the \inlinecode{read_write_access} handle, illustrated below. Again, we show an example done within the same task
and an example using a deferred task.
\begin{CppCode}
...
\end{CppCode}
As with \inlinecode{read_access}, value get/set should at least occur within a scoped code block,
and will likely perform better if placed in a deferred task block.
...
It may occur that the task is using a read-write handle, but create a task that only requires read access.
...
Without the \inlinecode{reads} qualifier, these tasks would conflict since they would by default request read-write privileges.

\subsection{Publish}
\label{subsec:publish}
By default, unless explicitly published, data visible only to tasks within the same scope.
...
Unpublished data will be reclaimed once the variable goes out of scope,
...
In order to reclaim the data (garbage collect or resolve anti-dependence), 
publish data must know its access group.
When all read handles within an access group have been deleted or released \emph{globally},
the published data can be reclaimed.
The easiest way to declare an access group (and currently supported method) is to simply give the total number of additional read handles.
...
Until all readers in an access group have received or released their data,
DARMA cannot reclaim (garbage collect, clear anti-dependencies).
...
\todo[inline]{This needs to somehow go after create\_work() is
introduced, the code examples don't actually work as is}
...
but needs to be created, 2) a handle to data that might already exist and needs to
be read, or 3) a handle to data that it wants to overwrite or modify. 
...
The second type of handle, requesting read-only access to a piece of data, is via the access handle \inlinecode{read_access}. 
...
//read-write work down here
\end{CppCode}
In this case, subtasks are created that only needs read access. 
Without the \inlinecode{reads} qualifier, these tasks would conflict since they would by default request read-write privileges.

\subsection{Publish}
\label{subsec:publish}
By default, unless explicitly published, data remains is only visible to tasks within the same scope.
For data to be globally visible in the global memory space (key-value store),
the application developer must explicitly \inlinecode{publish} data.
Unpublished data will be reclaimed once the variable goes out of scope,
freeing the memory and resolving any anti-dependencies analogous to the destructor invocation in C++ when a class goes out of scope.
...
\subsection{Keys}
\label{subsec:keys}
In the examples in this Section, the \inlinecode{key} to the accessor has always been a single string.  
In general, keys can be an arbitrary tuple of strings, integers, floats, or any type that implements a comparison operator and is bit-wise copiable data.  
The example at the end of Section~\ref{sec:spmd} illustrates the use of the \gls{rank} within the handle \inlinecode{key}.  
The following example shows the use of an arbitrary \gls{tuple} as a \inlinecode{key}:
\begin{CppCode}
  int neighbor_id = 42;
  double theta = PI / 6;  
  auto float_handle = initial_access<float>("float_key", 
